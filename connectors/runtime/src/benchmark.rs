...
pub const TARGET: &str = "picomq_connectors::benchmark";
...
mod tests {
    ...
    fn given_sink_event_emitted_when_captured_should_contain_all_fields() {
        let events = capture(|| {
            emit_sink_event("postgres", "records", 0, 100, 50, 50, 120, 234, 1456, 1700);
        });
        let event = events
            .iter()
    ...
    fn given_source_event_with_state_save_when_captured_should_contain_some_value() {
        ...
        assert_eq!(event.fields.get("broker_send_us").unwrap(), "567");
    ...
    fn given_source_event_without_state_save_when_captured_should_be_zero_and_unsaved() {
        let events = capture(|| {
            emit_source_event("random", "example_topic", 25, 23, 12, 34, 567, None, 750);
        });
        let event = events
            .iter()
            .find(|event| event.target == TARGET && event.message == "benchmark")
