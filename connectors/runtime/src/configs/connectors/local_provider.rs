...
use crate::configs::runtime::{ENV_NESTED_SEPARATOR, ENV_PREFIX};
        ...
        for entry in entries.flatten() {
            ...
            if path.is_file() {
                ...
                let connector_config: ConnectorConfig =
                    Self::load_connector_config(&path, &base_config)?;
...
impl<S: ProviderState> LocalConnectorsConfigProvider<S> {
    ...
    ) -> Result<ConnectorConfig, RuntimeError> {
        let prefix = Self::env_prefix(base_config);
        Figment::from(Toml::file(path))
            .merge(Env::prefixed(&prefix).split(ENV_NESTED_SEPARATOR))
            .extract()
            .map_err(|error| {
                RuntimeError::InvalidConfiguration(format!(
                    "Failed to load connector configuration from '{path}': {error}"
                ))
            })
    }

    fn env_prefix(base_config: &BaseConnectorConfig) -> String {
        format!(
            "{ENV_PREFIX}{}_{}_",
            base_config.connector_type().to_uppercase(),
            base_config.key().to_uppercase()
        )
    }
    ...
    fn apply_plugin_config_env_overrides(
        ...
        let prefix = format!("{}PLUGIN_CONFIG_", Self::env_prefix(base_config));
        ...
        for (env_key, env_value) in std::env::vars() {
            ...
            let parsed_value = parse_env_value_to_json(&env_value);
...
fn parse_env_value_to_json(value: &str) -> serde_json::Value {
    ...
    if looks_structured && let Ok(parsed) = serde_json::from_str::<serde_json::Value>(trimmed) {
        return parsed;
    }
