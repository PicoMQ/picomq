...
impl SinkManager {
    ...
    pub async fn update_status(
        ...
        let Some(sink) = self.sinks.get(key).map(|entry| entry.value().clone()) else {
            return;
        };
        {
    ...
    pub async fn sync_running_gauge(&self, metrics: &Arc<Metrics>) {
        ...
        for sink in self.sinks.iter().map(|entry| entry.value().clone()) {
            if sink.lock().await.info.status == ConnectorStatus::Running {
                running += 1;
            }
        }
        metrics.set_sinks_running(running);
    }
    ...
    pub async fn stop_connector(
        ...
        if let Some(container) = &container {
            ...
            (container.pico_sink_close)(plugin_id);
    ...
    pub async fn start_connector(
        ...
        let consumers = sink::setup_sink_consumers(key, config, kafka).await?;
...
mod tests {
    ...
    async fn should_clear_error_when_status_becomes_running() {
        let manager = SinkManager::new(vec![create_test_sink_details("es", 1)]);
        manager
            .set_error("es", "some error", &Arc::new(Metrics::init()))
            .await;
    ...
    async fn should_set_error_status_and_message() {
        ...
        manager
            .set_error("es", "connection failed", &Arc::new(Metrics::init()))
            .await;
