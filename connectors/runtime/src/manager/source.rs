...
use picomq_connector_sdk::api::{ConnectorError, ConnectorStatus};
...
impl SourceManager {
    ...
    pub async fn update_status(
        ...
        if let Some(metrics) = metrics {
            self.sync_running_gauge(metrics).await;
        }
    }

    pub async fn set_error(&self, key: &str, error_message: &str, metrics: &Arc<Metrics>) {
        let Some(source) = self.sources.get(key).map(|entry| entry.value().clone()) else {
            return;
        };
        {
    ...
    pub async fn clear_error(&self, key: &str, metrics: &Arc<Metrics>) {
        let Some(source) = self.sources.get(key).map(|entry| entry.value().clone()) else {
            return;
        };
        {
    ...
    pub async fn sync_running_gauge(&self, metrics: &Arc<Metrics>) {
        let mut running = 0u32;
        for source in self.sources.iter().map(|entry| entry.value().clone()) {
            if source.lock().await.info.status == ConnectorStatus::Running {
                running += 1;
            }
        }
        metrics.set_sources_running(running);
    }
    ...
    pub async fn stop_connector(
        ...
        if let Some(container) = &container {
            ...
            (container.pico_source_close)(plugin_id);
    ...
    pub async fn start_connector(
        ...
        let state = state_storage
            ...
            .map_err(|load_error| match load_error {
                picomq_connector_sdk::Error::TransientState(_)
                | picomq_connector_sdk::Error::PermanentState(_)
                | picomq_connector_sdk::Error::StateLatched => RuntimeError::StateLoadFailed {
        ...
        let (producer, transforms) = source::setup_source_producer(key, config, kafka).await?;
    ...
    pub async fn restart_connector(
        ...
        self.start_connector(key, &config, kafka, metrics, context)
            .await?;
...
mod tests {
    ...
    async fn should_clear_error_when_status_becomes_running() {
        let manager = SourceManager::new(vec![create_test_source_details("pg", 1)]);
        manager
            .set_error("pg", "some error", &Arc::new(Metrics::init()))
            .await;
    ...
    async fn should_set_error_status_and_message() {
        ...
        manager
            .set_error("pg", "connection failed", &Arc::new(Metrics::init()))
            .await;
    ...
    async fn set_error_should_be_noop_for_unknown_key() {
        ...
        manager
            .set_error("nonexistent", "some error", &Arc::new(Metrics::init()))
            .await;
    }
}
