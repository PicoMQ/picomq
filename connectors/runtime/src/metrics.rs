...
pub struct SourceLabels {
    ...
    pub stage_decode: StageLabels,
    pub stage_prepare: StageLabels,
    pub stage_broker_send: StageLabels,
...
impl SourceLabels {
    pub fn new(plugin_key: &str) -> Self {
        ...
        Self {
            ...
            stage_decode: stage(Stage::Decode),
            stage_prepare: stage(Stage::Prepare),
            stage_broker_send: stage(Stage::BrokerSend),
...
impl Metrics {
    pub fn init() -> Self {
        ...
        registry.register(
            "picomq_connectors_sources_total",
            "Total configured source connectors",
            sources_total.clone(),
        );
        registry.register(
            "picomq_connectors_sources_running",
            "Sources in Running status",
            sources_running.clone(),
        );
        registry.register(
            "picomq_connectors_sinks_total",
            "Total configured sink connectors",
            sinks_total.clone(),
        );
        registry.register(
            "picomq_connectors_sinks_running",
            "Sinks in Running status",
            sinks_running.clone(),
        );
        registry.register(
            "picomq_connector_messages_produced",
            "Messages received from source plugin poll",
            messages_produced.clone(),
        );
        registry.register(
            "picomq_connector_messages_sent",
            "Messages sent to PicoMQ (source)",
            messages_sent.clone(),
        );
        registry.register(
            "picomq_connector_messages_consumed",
            "Messages consumed from PicoMQ (sink)",
            messages_consumed.clone(),
        );
        registry.register(
            "picomq_connector_messages_processed",
            "Messages processed and sent to sink plugin",
            messages_processed.clone(),
        );
        registry.register(
            "picomq_connector_messages_filtered",
            "Messages intentionally dropped by transforms returning Ok(None)",
            messages_filtered.clone(),
        );
        registry.register(
            "picomq_connector_errors",
            "Errors encountered",
            errors.clone(),
        );
        registry.register(
            "picomq_connector_stage_duration_seconds",
    ...
    pub fn set_sources_running(&self, count: u32) {
        self.sources_running.set(i64::from(count));
    }

    pub fn set_sinks_running(&self, count: u32) {
        self.sinks_running.set(i64::from(count));
    }
    ...
    pub(crate) fn get_stage_duration_sample_count(
        ...
        let needle = format!(
            "picomq_connector_stage_duration_seconds_count{{connector_key=\"{key}\",connector_type=\"{type_label}\",stage=\"{stage_label}\"}}"
        );
        output
            .lines()
...
mod tests {
    ...
    const RENDERED_SERIES_NAMES: [&str; 13] = [
        ...
        "picomq_connectors_sinks_total",
        "picomq_connectors_sinks_running",
        "picomq_connector_messages_produced_total",
        "picomq_connector_messages_sent_total",
        "picomq_connector_messages_consumed_total",
        "picomq_connector_messages_processed_total",
        "picomq_connector_messages_filtered_total",
        "picomq_connector_errors_total",
        "picomq_connector_stage_duration_seconds_sum",
        "picomq_connector_stage_duration_seconds_count",
        "picomq_connector_stage_duration_seconds_bucket",
    ];
    ...
    fn test_metrics_init() {
        ...
        assert!(output.contains("picomq_connectors_sources_total"));
        assert!(output.contains("picomq_connectors_sources_running"));
        assert!(output.contains("picomq_connectors_sinks_total"));
        assert!(output.contains("picomq_connectors_sinks_running"));
    }
    ...
    fn test_set_sources_total() {
        ...
        let output = metrics.get_formatted_output();
        assert!(output.contains("picomq_connectors_sources_total 5"));
    }
    ...
    fn test_set_sinks_total() {
        ...
        let output = metrics.get_formatted_output();
        assert!(output.contains("picomq_connectors_sinks_total 3"));
    }
    ...
    fn test_increment_sources_running() {
        ...
        let output = metrics.get_formatted_output();
        assert!(output.contains("picomq_connectors_sources_running 2"));
    }
    ...
    fn test_decrement_sources_running() {
        ...
        let output = metrics.get_formatted_output();
        assert!(output.contains("picomq_connectors_sources_running 1"));
    }
    ...
    fn test_increment_sinks_running() {
        ...
        let output = metrics.get_formatted_output();
        assert!(output.contains("picomq_connectors_sinks_running 1"));
    }
    ...
    fn test_increment_messages_produced() {
        ...
        let output = metrics.get_formatted_output();
        assert!(output.contains("picomq_connector_messages_produced_total"));
    ...
    fn test_increment_messages_sent() {
        ...
        let output = metrics.get_formatted_output();
        assert!(output.contains("picomq_connector_messages_sent_total"));
        assert!(output.contains("connector_key=\"test-source\""));
    }
    ...
    fn test_increment_messages_consumed() {
        ...
        let output = metrics.get_formatted_output();
        assert!(output.contains("picomq_connector_messages_consumed_total"));
    ...
    fn test_increment_messages_processed() {
        ...
        let output = metrics.get_formatted_output();
        assert!(output.contains("picomq_connector_messages_processed_total"));
        assert!(output.contains("connector_key=\"test-sink\""));
    }
    ...
    fn test_increment_errors_source() {
        ...
        let output = metrics.get_formatted_output();
        assert!(output.contains("picomq_connector_errors_total"));
        assert!(output.contains("connector_key=\"test-source\""));
        assert!(output.contains("connector_type=\"source\""));
    }
    ...
    fn given_filtered_counter_when_incremented_should_register_under_labels() {
        ...
        let output = metrics.get_formatted_output();
        assert!(output.contains("picomq_connector_messages_filtered_total"));
    ...
    fn given_stage_histogram_when_encoded_should_expose_bucket_metrics() {
        ...
        let output = metrics.get_formatted_output();
        assert!(output.contains("picomq_connector_stage_duration_seconds"));
        assert!(output.contains("connector_key=\"es\""));
        assert!(output.contains("stage=\"broker_send\""));
        assert!(output.contains("picomq_connector_stage_duration_seconds_bucket"));
        assert!(output.contains("picomq_connector_stage_duration_seconds_count"));
        assert!(output.contains("picomq_connector_stage_duration_seconds_sum"));
    }
