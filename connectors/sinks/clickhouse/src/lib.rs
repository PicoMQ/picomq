...
use picomq_connector_sdk::{Error, sink_connector};
...
struct State {
    ...
    table_schemas: HashMap<String, Arc<[schema::Column]>>,
}
...
mod tests {
    ...
    fn given_templated_table_should_resolve_per_topic() {
        ...
        "#;
        let config: ClickHouseSinkConfig = toml::from_str(toml).unwrap();
        assert!(!config.table.is_static());
        assert_eq!(config.table.resolve("app.orders").unwrap(), "events_orders");
    }

    #[test]
    fn given_static_table_should_resolve_to_itself() {
        let config = test_config();
        assert!(config.table.is_static());
        assert_eq!(config.table.resolve("anything").unwrap(), "events");
    }
