...
use picomq_connector_sdk::{
    ...
    convert::owned_value_to_serde_json, now_millis, sink_connector,
};
...
const DEFAULT_MAX_PAYLOAD_SIZE: u64 = 10 * 1024 * 1024;
...
const URL_PROBE_TOPIC: &str = "topic";
...
struct MetadataEnvelope {
    metadata: PicoMQMetadata,
    payload: serde_json::Value,
}

#[derive(Debug, Serialize)]
struct PicoMQMetadata {
    pico_offset: u64,
    pico_timestamp: u64,
    pico_topic: String,
    pico_partition: i32,
    #[serde(skip_serializing_if = "Option::is_none")]
    pico_key: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pico_headers: Option<serde_json::Map<String, serde_json::Value>>,
}

#[derive(Debug, Serialize)]
struct EncodedPayload {
    data: String,
    pico_payload_encoding: &'static str,
}

#[derive(Debug, Serialize)]
struct EncodedHeader {
    data: String,
    pico_header_encoding: &'static str,
}

#[derive(Debug, Serialize, Deserialize)]
pub struct HttpSinkConfig {
    pub url: DestinationTemplate,
...
impl HttpSink {
    pub fn new(id: u32, config: HttpSinkConfig) -> Self {
        let url = config.url;
        let log_url = sanitize_url_for_log(&url.to_string());
        ...
        let include_key = config.include_key.unwrap_or(true);
        ...
        match payload {
            ...
            Payload::Raw(bytes) | Payload::FlatBuffer(bytes) => {
                let encoded = EncodedPayload {
                    ...
                    pico_payload_encoding: ENCODING_BASE64,
                };
    ...
    fn build_picomq_metadata(
        &self,
        message: &ConsumedMessage,
        topic_metadata: &TopicMetadata,
        messages_metadata: &MessagesMetadata,
    ) -> Result<PicoMQMetadata, Error> {
                    ...
                    let json_value = match std::str::from_utf8(value) {
                        ...
                        Err(_) => {
                            let encoded = EncodedHeader {
                                ...
                                pico_header_encoding: ENCODING_BASE64,
                            };
                            serde_json::to_value(encoded).map_err(|e| {
        ...
        Ok(PicoMQMetadata {
            pico_offset: message.offset,
            pico_timestamp: message.timestamp,
            pico_topic: topic_metadata.topic.clone(),
            pico_partition: messages_metadata.partition,
            pico_key: if self.include_key {
                message
                    .key
                    .as_deref()
    ...
    fn build_envelope(
        ...
        let metadata = self.build_picomq_metadata(message, topic_metadata, messages_metadata)?;
    ...
    async fn send_with_retry(
        ...
        let response = build_request(self.method, client, url)
        ...
        F: FnMut(ConsumedMessage) -> Result<Vec<u8>, Error>,
    {
        ...
        for message in messages {
            ...
            match self
                .send_with_retry(url, Bytes::from(body), content_type)
                .await
            {
                Ok(()) => {
                    delivered += 1;
    ...
    async fn send_batch_body(
        ...
        if let Err(e) = self
            .send_with_retry(url, body, self.batch_mode.content_type())
            .await
        {
            self.errors_count
    ...
    async fn send_ndjson(
        ...
        self.send_batch_body(url, Bytes::from(body), count, skipped)
            .await
    }
    ...
    async fn send_raw(&self, url: &str, messages: Vec<ConsumedMessage>) -> Result<(), Error> {
        self.send_per_message(url, messages, self.batch_mode.content_type(), |message| {
            message
                .payload
                .try_into_vec()
...
fn validate_url(url: &str, log_url: &str) -> Result<(), Error> {
...
impl Sink for HttpSink {
    async fn open(&mut self) -> Result<(), Error> {
        ...
        let url_template = self.url.to_string();
        if url_template.is_empty() {
            return Err(Error::InitError(
                "HTTP sink URL is empty - 'url' is required in [plugin_config]".to_string(),
            ));
        }
        let probe_url = self.url.resolve(URL_PROBE_TOPIC)?;
        validate_url(&probe_url, &self.log_url)?;
        ...
        if self.health_check_enabled && !self.url.is_static() {
            warn!(
                "HTTP sink ID: {} - health check skipped because url '{}' is templated per topic",
                self.id, self.log_url
            );
        } else if self.health_check_enabled {
            let client = self.client.as_ref().expect("client just built");
            let headers = self
                .request_headers
                .as_ref()
                .expect("request_headers just built");
            let health_request = build_request(self.health_check_method, client, &probe_url)
                .headers(headers.clone());
    ...
    async fn consume(
        ...
        if self.verbose {
            debug!(
                "HTTP sink ID: {} - received {} messages (schema: {}, topic: {})",
                self.id, messages_count, messages_metadata.schema, topic_metadata.topic,
            );
        }

        let url = self.url.resolve(&topic_metadata.topic)?;
        if !self.url.is_static() {
            validate_url(&url, &sanitize_url_for_log(&url))?;
        }

        let result = match self.batch_mode {
            BatchMode::Individual => {
                self.send_individual(&url, topic_metadata, &messages_metadata, messages)
                    .await
            }
            BatchMode::NdJson => {
                self.send_ndjson(&url, topic_metadata, &messages_metadata, messages)
                    .await
            }
            BatchMode::JsonArray => {
                self.send_json_array(&url, topic_metadata, &messages_metadata, messages)
                    .await
            }
            BatchMode::Raw => self.send_raw(&url, messages).await,
        };
...
mod tests {
    use super::*;
    use picomq_connector_sdk::{Headers, Schema};

    const FIELD_DATA: &str = "data";
    const FIELD_PAYLOAD_ENCODING: &str = "pico_payload_encoding";
    const FIELD_METADATA: &str = "metadata";
    const FIELD_PAYLOAD: &str = "payload";
    const FIELD_OFFSET: &str = "pico_offset";
    const FIELD_TIMESTAMP: &str = "pico_timestamp";
    const FIELD_TOPIC: &str = "pico_topic";
    const FIELD_PARTITION_ID: &str = "pico_partition";
    const FIELD_KEY: &str = "pico_key";
    const FIELD_HEADERS: &str = "pico_headers";
    ...
    #[test]
    fn given_templated_url_should_resolve_per_topic() {
        let mut config = given_default_config();
        config.url = "https://api.example.com/{topic_segment[0]}/{topic}"
            .parse()
            .unwrap();
        let sink = HttpSink::new(1, config);
        assert!(!sink.url.is_static());
        assert_eq!(
            sink.url.resolve("orders.created").unwrap(),
            "https://api.example.com/orders/orders.created"
        );
    }

    #[test]
    fn given_templated_url_with_bad_scheme_should_fail_validation() {
        let url = "ftp://host/{topic}".parse::<DestinationTemplate>().unwrap();
        let resolved = url.resolve("topic").unwrap();
        assert!(validate_url(&resolved, &resolved).is_err());
    }
    ...
    fn given_include_metadata_true_should_wrap_payload() {
        let sink = given_sink_with_defaults();
        let message = given_json_message(10);
        ...
        assert_eq!(metadata[FIELD_TIMESTAMP], 1710064800000u64);
        assert_eq!(metadata[FIELD_TOPIC], "test_topic");
        assert_eq!(metadata[FIELD_PARTITION_ID], 0);
        assert_eq!(
            metadata[FIELD_KEY],
            general_purpose::STANDARD.encode(b"user-1")
        );
    }
    ...
    fn given_include_metadata_false_should_return_raw_payload() {
        ...
        let message = given_json_message(0);
        let topic_meta = given_topic_metadata();
        let msg_meta = given_messages_metadata();
        let payload_json = sink.payload_to_json(message.payload.clone()).unwrap();
    ...
    #[test]
    fn given_include_key_false_should_omit_key_from_metadata() {
        let mut config = given_default_config();
        config.include_key = Some(false);
        let sink = HttpSink::new(1, config);
        ...
        let envelope = sink
            .build_envelope(&message, &topic_meta, &msg_meta, payload_json)
            .unwrap();
        assert!(envelope[FIELD_METADATA].get(FIELD_KEY).is_none());
    }

    #[test]
    fn given_message_with_headers_should_include_pico_headers_in_metadata() {
        let sink = given_sink_with_defaults();
        let topic_meta = given_topic_metadata();
        let msg_meta = given_messages_metadata();

        let mut headers = Headers::new();
        headers.insert("x-correlation-id".to_string(), b"abc-123".to_vec());
        headers.insert("x-binary".to_string(), vec![0xff, 0xfe]);
        ...
        let pico_headers = &envelope[FIELD_METADATA][FIELD_HEADERS];
        assert_eq!(pico_headers["x-correlation-id"], "abc-123");
        assert_eq!(pico_headers["x-binary"][FIELD_DATA], "//4=");
        assert_eq!(pico_headers["x-binary"]["pico_header_encoding"], "base64");
        assert!(envelope[FIELD_METADATA].get(FIELD_KEY).is_none());
    }

    #[test]
    fn given_message_without_headers_should_not_include_pico_headers() {
        let sink = given_sink_with_defaults();
        let message = given_json_message(0);
        let topic_meta = given_topic_metadata();
        let msg_meta = given_messages_metadata();
        ...
        assert!(
            ...
            "Expected no pico_headers when message has no headers"
        );
    }
    ...
    fn given_minimal_toml_config_should_deserialize() {
        ...
        assert_eq!(config.url.to_string(), "https://example.com");
    ...
    fn given_full_toml_config_should_deserialize_all_fields() {
        ...
        let config: HttpSinkConfig = toml::from_str(toml_str).unwrap();
        assert_eq!(config.url.to_string(), "https://example.com/api");
    ...
    async fn given_empty_url_should_fail_open() {
        let mut config = given_default_config();
        config.url = DestinationTemplate::literal("");
    ...
    async fn given_invalid_url_should_fail_open() {
        let mut config = given_default_config();
        config.url = "not a url".parse().unwrap();
        let mut sink = HttpSink::new(1, config);
        let result = sink.open().await;
        assert!(result.is_err());
        let err = result.unwrap_err().to_string();
    ...
    async fn given_file_scheme_url_should_fail_open() {
        let mut config = given_default_config();
        config.url = "file:///etc/passwd".parse().unwrap();
        let mut sink = HttpSink::new(1, config);
        let result = sink.open().await;
        assert!(result.is_err());
        let err = result.unwrap_err().to_string();
    ...
    async fn given_ftp_scheme_url_should_fail_open() {
        let mut config = given_default_config();
        config.url = "ftp://fileserver.local/data".parse().unwrap();
        let mut sink = HttpSink::new(1, config);
        let result = sink.open().await;
        assert!(result.is_err());
        let err = result.unwrap_err().to_string();
    ...
    async fn given_http_scheme_url_should_pass_open() {
        let mut config = given_default_config();
        config.url = "http://localhost:8080/ingest".parse().unwrap();
        let mut sink = HttpSink::new(1, config);
        sink.health_check_enabled = false;
        let result = sink.open().await;
        assert!(result.is_ok());
    ...
    async fn given_consume_called_before_open_should_return_init_error() {
        ...
        let messages = vec![given_json_message(0)];
        let result = sink
    ...
    fn given_default_config() -> HttpSinkConfig {
        HttpSinkConfig {
            url: "https://api.example.com/ingest".parse().unwrap(),
            method: None,
            timeout: None,
    ...
    fn given_json_message(offset: u64) -> ConsumedMessage {
        ConsumedMessage {
            offset,
            timestamp: 1710064800000,
