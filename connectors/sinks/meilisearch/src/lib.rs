...
const DEFAULT_PRIMARY_KEY: &str = "pico_id";
...
impl MeilisearchSink {
    ...
    fn resolve_index(&self, topic: &str) -> Result<String, Error> {
        let resolved = self.config.index.resolve(topic)?;
        let sanitized = sanitize_index_uid(&resolved);
        if sanitized.is_empty() {
            return Err(Error::InvalidConfigValue(format!(
                "Meilisearch index resolved from topic '{topic}' is empty"
            )));
        }
        Ok(sanitized)
    }
    ...
    async fn ensure_index_exists(&self, client: &Client, index_uid: &str) -> Result<(), Error> {
        if self.ensured_indexes.lock().await.contains(index_uid) {
            return Ok(());
        }
        match self.get_index_if_exists(client, index_uid).await? {
            Some(index) => {
                info!("Meilisearch index '{index_uid}' already exists");
                if let Some(primary_key) = index.primary_key.as_deref()
                    && primary_key != self.config.primary_key
                {
                    warn!(
                        "Meilisearch index '{index_uid}' primary key '{primary_key}' differs from configured primary key '{}'",
                        self.config.primary_key
                    );
                } else if index.primary_key.is_none() {
                    warn!(
                        "Meilisearch index '{index_uid}' does not currently have a primary key. Configured primary key '{}' will be sent with document indexing requests.",
                        self.config.primary_key
                    );
                }
            }
            None if self.config.create_index_if_not_exists => {
                self.create_index(client, index_uid).await?
            }
            None => {
                return Err(Error::InitError(format!(
                    "Meilisearch index '{index_uid}' does not exist and create_index_if_not_exists=false"
                )));
            }
        }
    ...
    async fn get_index_if_exists(
        ...
        self.retry_sdk_open_operation("get index", || async {
            match client.get_index(index_uid).await {
    ...
    async fn create_index(&self, client: &Client, index_uid: &str) -> Result<(), Error> {
        info!(
            "Creating Meilisearch index '{index_uid}' with primary key '{}'",
            self.config.primary_key
        );

        let task = self
            .retry_sdk_open_operation("create index", || {
                client.create_index(index_uid, Some(&self.config.primary_key))
            })
            .await?;
        self.wait_for_index_creation_task(client, task).await?;

        info!("Created Meilisearch index '{index_uid}'");
        Ok(())
    }
        ...
        if self.config.include_metadata {
            if self.config.primary_key != DEFAULT_PRIMARY_KEY {
                upsert_metadata_field(
                    &mut document,
                    DEFAULT_PRIMARY_KEY,
                    Value::String(generated_id),
                );
            }
            upsert_metadata_field(&mut document, "pico_offset", Value::from(offset));
            upsert_metadata_field(
                &mut document,
                "pico_topic",
            ...
            upsert_metadata_field(
                &mut document,
                "pico_partition",
                Value::from(messages_metadata.partition),
            );
            upsert_metadata_field(&mut document, "pico_timestamp", Value::from(timestamp));
            upsert_metadata_field(
                &mut document,
                "pico_ingested_at",
                Value::from(now_millis() as i64),
            );
            if let Some(key) = &key {
                upsert_metadata_field(
                    &mut document,
            ...
            if let Some(headers) = &headers {
                let rendered: Map<String, Value> = headers
                    .iter()
                    .map(|(name, value)| {
                        (
                            name.clone(),
                            Value::String(general_purpose::STANDARD.encode(value)),
                        )
                    })
                    .collect();
                upsert_metadata_field(&mut document, "pico_headers", Value::Object(rendered));
            }
        }
    ...
    async fn index_documents(
        ...
        for chunk in documents.chunks(self.config.batch_size) {
            match self.index_document_chunk(client, index_uid, chunk).await {
                Ok(indexed) => {
                    accepted += indexed;
                    accounted += chunk.len();
                }
...
impl Sink for MeilisearchSink {
    async fn open(&mut self) -> Result<(), Error> {
        ...
        if self.config.index.is_static() {
            let index_uid = self.resolve_index("")?;
            self.ensure_index_exists(&client, &index_uid).await?;
        }
    ...
    async fn consume(
        ...
        info!(
            "Meilisearch sink with ID: {} received: {} messages, schema: {}, topic: {}, partition: {}, offset: {}, invocation: {}",
            self.id,
            messages.len(),
            messages_metadata.schema,
        ...
        let index_uid = self.resolve_index(&topic_metadata.topic)?;
        self.ensure_index_exists(client, &index_uid).await?;
        ...
        match self.index_documents(client, &index_uid, documents).await {
            Ok(accepted) => {
                let mut state = self.state.lock().await;
                ...
                info!(
                    "Accepted {accepted} of {messages_count} messages into Meilisearch index '{index_uid}'"
                );
                Ok(())
            }
            Err(partial_error) => {
                ...
                drop(state);
                error!(
                    "Failed to index Meilisearch sink batch for connector ID: {}, index: {index_uid}, accepted: {}, failed: {}, error: {}",
                    self.id, partial_error.accepted, partial_error.failed, partial_error.error
                );
...
fn generated_document_id(
    ...
    generated_document_id_from_parts(topic_metadata, messages_metadata, message.offset)
...
fn generated_document_id_from_parts(
    ...
    Ok(format!("pico_{encoded}"))
}

fn sanitize_index_uid(raw: &str) -> String {
    raw.trim()
        .chars()
        .map(|ch| {
...
mod tests {
    ...
    #[test]
    fn templated_index_resolves_and_sanitizes_topic() {
        let mut config = base_config();
        config.index = "idx_{topic}".parse().unwrap();
        let sink = sink_with_config(config);

        assert_eq!(
            sink.resolve_index("orders.us/east").unwrap(),
            "idx_orders_us_east"
        );
    }

    #[test]
    fn static_index_is_validated_on_open() {
        let mut config = base_config();
        config.index = "  ".parse().unwrap();
        let sink = sink_with_config(config);
    ...
    fn injects_default_primary_key_and_metadata() {
        ...
        assert_eq!(document["pico_id"], expected_id);
        assert_eq!(document["pico_offset"], 11);
        assert_eq!(document["pico_topic"], "created/topic");
        assert_eq!(document["pico_partition"], 7);
        assert_eq!(document["pico_key"], "azE=");
    }
    ...
    fn omits_metadata_when_include_metadata_is_false() {
        ...
        assert_eq!(document["name"], "Alice");
        assert!(document["pico_id"].as_str().is_some());
        assert!(document.get("pico_offset").is_none());
        assert!(document.get("pico_topic").is_none());
    }
