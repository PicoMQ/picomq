...
use bytes::Bytes;
use picomq_connector_sdk::convert::owned_value_to_serde_json;
use picomq_connector_sdk::destination::DestinationTemplate;
use picomq_connector_sdk::retry::{exponential_backoff, jitter, parse_duration};
use picomq_connector_sdk::{
    ConsumedMessage, Error, Headers, MessagesMetadata, Payload, Sink, TopicMetadata, sink_connector,
};
...
impl Sink for SurrealDbSink {
    async fn open(&mut self) -> Result<(), Error> {
        ...
        if self.auto_define_table && self.table.is_static() {
            let table = self.resolve_table("")?;
            self.ensure_table(&client, &table).await?;
        }
...
impl SurrealDbSink {
    async fn connect_and_select(&self) -> Result<SurrealDbClient, Error> {
        ...
        if self.auto_define_table {
            ...
            self.ensured_tables.lock().await.clear();
        }

        Ok(client)
    }

    fn resolve_table(&self, topic: &str) -> Result<String, Error> {
        let table = sanitize_identifier(&self.table.resolve(topic)?);
    ...
    async fn ensure_table(&self, client: &SurrealDbClient, table: &str) -> Result<(), Error> {
        ...
        if self.define_indexes {
            let offset_index = format!("{table}_pico_offset_idx");
            validate_identifier("index", &offset_index)?;
            query.push_str(&format!(
                " DEFINE INDEX IF NOT EXISTS {offset_index} ON TABLE {table} FIELDS pico_topic, pico_partition, pico_offset;"
            ));
        }
    ...
    async fn process_messages(
        ...
        let table = self.resolve_table(&topic_metadata.topic)?;
        if self.auto_define_table {
        ...
        for message in messages {
            ...
            if batch.len() == self.batch_size {
                ...
                if let Some(batch_error) = outcome.error {
                    ...
                    error!(
                        "Failed to insert SurrealDB batch of {batch_len} messages for connector ID: {}, table: {table}, error: {batch_error}",
                        self.id
                    );
        ...
        if self.verbose {
            info!(
                "SurrealDB sink ID: {} wrote {successful_inserts} messages to table '{table}'",
                self.id
            );
        } else {
            debug!(
    ...
    async fn insert_batch(
        ...
        let mut outcome = self.insert_records_with_retry(table, records).await;
    ...
    async fn insert_records_with_retry(
        ...
        let query = match build_insert_query(table, &records) {
            Ok(query) => query,
            Err(error) => {
                return BatchInsertOutcome {
                    inserted_count: 0,
    ...
    fn build_record(
        ...
        if self.include_metadata {
            ...
            record.insert(
                ...
                Value::String(messages_metadata.partition.to_string()),
            );
            record.insert(
                "pico_offset".to_string(),
            ...
            && let Some(key) = &message.key
        {
            record.insert(
                "pico_key".to_string(),
                Value::String(general_purpose::STANDARD.encode(key)),
            );
        }
        ...
        if self.include_headers
            ...
            && !headers.is_empty()
        {
            record.insert("pico_headers".to_string(), encode_headers(headers));
        }
...
fn encode_headers(headers: &Headers) -> Value {
    ...
    for (key, value) in headers {
        ...
        encoded.insert(key.clone(), value);
    }
...
fn build_record_id(
    ...
    let mut id = String::with_capacity(record_id_prefix.topic.len() + 40);
    id.push('t');
    id.push_str(&record_id_prefix.topic);
    id.push_str("_p");
    id.push_str(&messages_metadata.partition.to_string());
    id.push_str("_o");
...
mod tests {
    ...
    fn test_config() -> SurrealDbSinkConfig {
        SurrealDbSinkConfig {
            ...
            namespace: "picomq".to_string(),
            database: "connectors".to_string(),
            table: "picomq_messages".parse().unwrap(),
    ...
    fn given_identifier_values_should_validate_expected_shapes() {
        assert!(validate_identifier("table", "picomq_messages").is_ok());
    ...
    fn given_topic_metadata_should_build_deterministic_record_id() {
        ...
        let record_id_prefix = RecordIdPrefix::new(&topic_metadata);
        let id = build_record_id(&record_id_prefix, &test_messages_metadata(), 9);

        assert_eq!(id, "t746573745f746f706963_p7_o9");
    }

    #[test]
    fn given_templated_table_should_resolve_sanitized_identifier() {
        let mut config = test_config();
        config.table = "events_{topic}".parse().unwrap();
        let sink = SurrealDbSink::new(1, config);

        assert_eq!(
            sink.resolve_table("orders.us-east").unwrap(),
            "events_orders_us_east"
        );
    }

    #[test]
    fn given_topic_starting_with_digit_should_prefix_identifier() {
        let mut config = test_config();
        config.table = "{topic}".parse().unwrap();
        let sink = SurrealDbSink::new(1, config);

        assert_eq!(sink.resolve_table("42_events").unwrap(), "_42_events");
    }
    ...
    fn given_table_name_should_build_bulk_insert_query() {
        ...
        assert_eq!(
            String::from_utf8(
                build_insert_query("picomq_messages", &records)
                    .expect("query should build")
                    .to_vec()
            )
            .expect("query should be valid UTF-8"),
            r#"INSERT IGNORE INTO picomq_messages [{"id":"record_1","payload":{"message":"hello"}}] RETURN NONE;"#
        );
    }
    ...
    fn given_adversarial_record_values_should_build_escaped_insert_query() {
        ...
        .expect("query should be valid UTF-8");
        let json_start = "INSERT IGNORE INTO picomq_messages ";
    ...
    #[test]
    fn given_headers_should_encode_binary_as_base64_and_utf8_as_strings() {
        let mut headers = Headers::new();
        headers.insert("trace-id".to_string(), b"abc".to_vec());
        headers.insert("binary".to_string(), vec![0xff, 0xfe, 0xfd]);
    ...
    fn given_message_should_build_full_record() {
        ...
        let mut headers = Headers::new();
        headers.insert("source".to_string(), b"unit-test".to_vec());
        ...
        assert_eq!(
            object.get("id"),
            Some(&Value::String("t746573745f746f706963_p7_o9".to_string()))
        );
        assert_eq!(object.get("pico_topic"), Some(&json!("test_topic")));
        assert_eq!(object.get("pico_partition"), Some(&json!("7")));
        assert_eq!(object.get("pico_offset"), Some(&json!("9")));
        assert_eq!(object.get("pico_timestamp"), Some(&json!("1700000000000")));
        assert_eq!(object.get("pico_key"), Some(&json!("azE=")));
        assert_eq!(object.get("payload"), Some(&json!({"event": "created"})));
        assert_eq!(object.get("payload_encoding"), Some(&json!("json")));
        assert!(object.contains_key("pico_headers"));
    }
    ...
    fn given_large_u64_metadata_should_build_record_with_lossless_strings() {
        ...
        assert_eq!(
            object.get("pico_offset"),
            Some(&json!("18446744073709551615"))
        );
        assert_eq!(
            object.get("pico_timestamp"),
            Some(&json!("18446744073709551615"))
        );
    }
    ...
    fn given_metadata_disabled_should_build_minimal_record() {
        ...
        assert!(!object.contains_key("pico_topic"));
        assert!(!object.contains_key("pico_key"));
        assert!(!object.contains_key("pico_headers"));
    }
}
