...
impl Sink for S3Sink {
    ...
    async fn consume(
        ...
        let key = BufferKey {
            ...
            partition: messages_metadata.partition,
        };
        ...
        if let Err(ref e) = result {
            ...
            if lost > 0 {
                ...
                error!(
                    ...
                    self.id, topic_metadata.topic, messages_metadata.partition,
                );
            }
        }
        ...
        debug!(
            ...
            self.id, batch_size, topic_metadata.topic, messages_metadata.partition,
        );
                ...
                if let Some(Ok(payload)) = flush_payload {
                    if let Err(e) = self.do_upload(bucket, payload).await {
                        error!(
                            ...
                            self.id, key.topic, key.partition
                        );
                    }
...
impl S3Sink {
    ...
    fn extract_flush_payload(
        ...
        let ctx = PathContext {
            ...
            first_timestamp_millis: buffer.first_timestamp_millis(),
        };
...
mod tests {
    ...
    #[test]
    fn extract_flush_payload_embeds_partition() {
        ...
        buffer.append(b"{\"a\":1}", 0, 1_710_597_600_000);
        buffer.append(b"{\"b\":2}", 1, 1_710_597_601_000);

        let payload = sink.extract_flush_payload(&key, &mut buffer).unwrap();
        assert!(
            payload.s3_key.contains("00007-"),
            "S3 key must contain partition: {}",
            payload.s3_key
        );
