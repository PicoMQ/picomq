...
use picomq_connector_sdk::{
    ConsumedMessage, Error, Headers, MessagesMetadata, Payload, TopicMetadata,
...
fn format_json_message(
    ...
    let msg = JsonMessage {
        ...
        partition: if include_metadata {
            Some(messages_metadata.partition)
        } else {
            None
        },
        key: if include_metadata {
            message.key.as_deref().map(base64_encode)
        } else {
            None
        },
...
fn timestamp_to_rfc3339(millis: u64) -> String {
    i64::try_from(millis)
        .ok()
...
mod tests {
    ...
    fn json_lines_with_headers() {
        ...
        headers.insert("binary".to_string(), vec![0xff, 0xfe]);
        ...
        assert_eq!(value["headers"]["binary"], "//4=");
    }
    ...
    fn timestamp_conversion() {
        let ts = timestamp_to_rfc3339(1_710_597_751_123);
        assert_eq!(ts, "2024-03-16T14:02:31.123Z");
    }
