...
pub struct MongoDbSinkConfig {
    ...
    pub database: String,
    pub collection: DestinationTemplate,
...
impl Sink for MongoDbSink {
    async fn open(&mut self) -> Result<(), Error> {
        ...
        if self.config.collection.is_static() {
            let collection = self.config.collection.resolve("")?;
...
impl MongoDbSink {
    ...
    async fn ensure_collection_exists(&self, collection: &str) -> Result<(), Error> {
        if !self.config.auto_create_collection.unwrap_or(false) {
            return Ok(());
        }
        if self.ensured_collections.lock().await.contains(collection) {
            return Ok(());
        }
        ...
        if existing.iter().any(|name| name == collection) {
            debug!("Collection '{collection}' already exists, skipping creation");
        } else {
            db.create_collection(collection).await.map_err(|e| {
                Error::InitError(format!("Failed to create collection '{collection}': {e}"))
            })?;
            info!("Created MongoDB collection '{collection}'");
        }
    ...
    async fn process_messages(
        ...
        let client = self.get_client()?;
        let coll = self.config.collection.resolve(&topic_metadata.topic)?;
        self.ensure_collection_exists(&coll).await?;
        let db = client.database(&self.config.database);
    ...
    async fn insert_batch(
        ...
        for message in messages {
            ...
            let document_id =
                build_composite_document_id(topic_metadata, messages_metadata, message.offset);
            ...
            if self.include_metadata {
                ...
                doc.insert("pico_topic", &topic_metadata.topic);
                doc.insert(
                    "pico_partition",
                    bson::Bson::Int32(messages_metadata.partition),
                );
            }
...
fn build_composite_document_id(
    ...
    format!(
        "{}:{}:{offset}",
        topic_metadata.topic, messages_metadata.partition
    )
}

fn build_offset_metadata_value(offset: u64) -> (&'static str, bson::Bson) {
    if let Ok(offset) = i64::try_from(offset) {
        ("pico_offset", bson::Bson::Int64(offset))
    } else {
        ("pico_offset_str", bson::Bson::String(offset.to_string()))
    }
}

fn build_bson_datetime_value(timestamp_millis: u64) -> bson::DateTime {
    bson::DateTime::from_millis(i64::try_from(timestamp_millis).unwrap_or(i64::MAX))
}
...
mod tests {
    ...
    fn given_default_config() -> MongoDbSinkConfig {
        MongoDbSinkConfig {
            ...
            collection: "test_collection".parse().unwrap(),
            max_pool_size: None,
    ...
    fn given_offset_values_should_use_expected_metadata_field() {
        ...
        assert_eq!(normal_key, "pico_offset");
        ...
        assert_eq!(oversized_key, "pico_offset_str");
        assert_eq!(
            oversized_value,
    ...
    fn given_timestamp_values_should_use_bounded_bson_datetime() {
        assert_eq!(
            build_bson_datetime_value(1_234),
            bson::DateTime::from_millis(1_234)
        );

        assert_eq!(
            build_bson_datetime_value(u64::MAX),
            bson::DateTime::from_millis(i64::MAX)
        );
    }

    #[test]
    fn given_templated_collection_should_resolve_per_topic() {
        let mut config = given_default_config();
        config.collection = "events_{topic_segment[0]}".parse().unwrap();
        assert_eq!(
            config.collection.resolve("orders.user42").unwrap(),
            "events_orders"
        );
    }
    ...
    fn given_cross_topic_messages_should_build_unique_document_ids() {
        ...
        let messages_metadata = MessagesMetadata {
            partition: 0,
            current_offset: 0,
            schema: picomq_connector_sdk::Schema::Raw,
        };
        ...
        assert_eq!(first_id, "topic_primary:0:42");
    ...
    async fn given_no_client_should_return_error_not_silent_ok() {
        ...
        let messages = vec![ConsumedMessage {
            ...
            key: None,
            headers: None,
            payload: picomq_connector_sdk::Payload::Raw(vec![1, 2, 3]),
        }];
