...
use picomq_connector_sdk::{
    ConnectorState, Error, ProducedMessage, ProducedMessages, Schema, Source, now_millis,
    source_connector,
};
...
const DEFAULT_REPLICATION_SLOT: &str = "picomq_slot";
...
impl FromStr for PayloadFormat {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        match value.to_lowercase().as_str() {
            ...
            "bytea" | "raw" => Ok(PayloadFormat::Bytea),
            "text" => Ok(PayloadFormat::Text),
            "json_direct" | "jsonb" | "jsonb_direct" => Ok(PayloadFormat::JsonDirect),
            other => Err(Error::InvalidConfigValue(format!(
                "Unsupported payload_format '{other}'. Use 'json', 'bytea'/'raw', 'text', or 'json_direct'/'jsonb'/'jsonb_direct'"
            ))),
        }
    }
}

impl From<PayloadFormat> for Schema {
...
enum PendingApply {
    Tables(Vec<TableCleanup>),
    SlotAdvance { slot_name: String, lsn: String },
}
...
impl Source for PostgresSource {
    ...
    async fn poll(&self) -> Result<ProducedMessages, Error> {
        ...
        let staged = match self.config.mode.as_str() {
    ...
    async fn on_batch_result(&self, result: SourceBatchResult) -> Result<(), Error> {
        ...
        match result {
            SourceBatchResult::Nack => {
                warn!(
                    "PostgreSQL source connector ID: {} batch was not accepted, discarding staged cursor and cleanup so the rows are re-read",
                    self.id
                );
                Ok(())
            }
            SourceBatchResult::Ack => {
                *self.state.lock().await = pending.state;
                self.apply_pending(pending.apply).await
            }
        }
    }
...
impl PostgresSource {
    ...
    async fn stage_batch(&self, staged: Option<StagedBatch>) -> Result<ProducedMessages, Error> {
        let schema = Schema::from(self.payload_format());
        let Some(staged) = staged else {
            debug!(
                "PostgreSQL source connector ID: {} produced no messages",
                self.id
            );
        ...
        let persisted_state = self.serialize_state(&staged.state).ok_or_else(|| {
        ...
        if self.verbose {
            info!(
                "PostgreSQL source connector ID: {} produced {} messages. Total processed after this batch: {}",
                self.id,
                staged.messages.len(),
                staged.state.processed_rows
            );
        } else {
            debug!(
                "PostgreSQL source connector ID: {} produced {} messages. Total processed after this batch: {}",
        ...
        *self.pending.lock().await = Some(PendingBatch {
            state: staged.state,
            apply: staged.apply,
        });
    ...
    async fn apply_pending(&self, apply: PendingApply) -> Result<(), Error> {
        match apply {
            PendingApply::Tables(cleanups) => {
                for cleanup in cleanups {
                    self.mark_or_delete_processed_rows(&cleanup).await?;
                }
                Ok(())
            }
            PendingApply::SlotAdvance { slot_name, lsn } => {
                self.advance_slot(&slot_name, &lsn).await
            }
        }
    }

    async fn poll_cdc(&self) -> Result<Option<StagedBatch>, Error> {
    ...
    async fn poll_cdc_builtin(&self) -> Result<Option<StagedBatch>, Error> {
        let pool = self.get_pool()?;
        ...
        for row in rows {
            let lsn: String = row.try_get("lsn").map_err(|e| {
                error!("CDC row has unreadable lsn column: {e}");
                Error::InvalidRecord
            })?;
            last_lsn = Some(lsn);
        ...
        Ok(Some(StagedBatch {
            messages,
            state,
            apply: PendingApply::SlotAdvance {
    ...
    async fn advance_slot(&self, slot_name: &str, lsn: &str) -> Result<(), Error> {
        let pool = self.get_pool()?;

        if self.verbose {
            info!("CDC: Advancing replication slot '{slot_name}' to {lsn}");
        } else {
            debug!("CDC: Advancing replication slot '{slot_name}' to {lsn}");
        }
    ...
    async fn poll_tables(&self) -> Result<Option<StagedBatch>, Error> {
        let pool = self.get_pool()?;
        let mut messages = Vec::new();
        ...
        for table in &self.config.tables {
            ...
            if !processed_ids.is_empty() && self.cleans_up_processed_rows() {
                cleanups.push(TableCleanup {
                    table: table.clone(),
            ...
            state.processed_rows += fetched;

            if self.verbose {
                info!("Fetched {fetched} rows from table '{table}'");
            } else {
                debug!("Fetched {fetched} rows from table '{table}'");
            }
        }
        ...
        Ok(Some(StagedBatch {
            messages,
            state,
            apply: PendingApply::Tables(cleanups),
        }))
    }

    fn cleans_up_processed_rows(&self) -> bool {
        self.config.delete_after_read.unwrap_or(false) || self.config.processed_column.is_some()
    }

    async fn mark_or_delete_processed_rows(&self, cleanup: &TableCleanup) -> Result<(), Error> {
        if cleanup.ids.is_empty() {
            return Ok(());
        }

        let pool = self.get_pool()?;
        let table = cleanup.table.as_str();
        let quoted_table = quote_qualified_identifier(table)?;
        let quoted_pk = quote_identifier(&cleanup.pk_column)?;

        let ids_list = cleanup
            .ids
            .iter()
            .map(|id| {
        ...
        let (query, action) = if self.config.delete_after_read.unwrap_or(false) {
            (
        ...
        if self.verbose {
            info!(
                "Applying {action} to {} acknowledged rows in '{table}'",
                cleanup.ids.len()
            );
        } else {
            debug!(
        ...
        .map_err(|_| {
            Error::Storage(format!(
                "failed to {action} {} acknowledged rows in '{table}'",
                cleanup.ids.len()
            ))
        })?;
    ...
    fn slot_name(&self) -> &str {
        self.config
            .replication_slot
            .as_deref()
            .unwrap_or(DEFAULT_REPLICATION_SLOT)
    }
    ...
    fn payload_format(&self) -> PayloadFormat {
            ...
            && !payload_col.is_empty()
        {
            return self
                ...
                .payload_format
                .as_deref()
                .and_then(|format| PayloadFormat::from_str(format).ok())
                .unwrap_or_default();
        }
        PayloadFormat::Json
    }
    ...
    match type_name {
        ...
        "BYTEA" => {
            let value: Option<Vec<u8>> = row
                ...
                .map_err(|_| Error::InvalidRecord)?;
            Ok(value
                .map(|bytes| serde_json::Value::String(BASE64_STANDARD.encode(&bytes)))
                .unwrap_or(serde_json::Value::Null))
        }
        "BOOL[]" => {
        ...
        _ => {
            ...
            match raw.as_str() {
                ...
                Err(_) => {
                    ...
                    Ok(serde_json::Value::String(BASE64_STANDARD.encode(bytes)))
                }
            }
        }
    }
...
fn validate_payload_format(payload_format: Option<&str>) -> Result<(), Error> {
    let Some(format) = payload_format else {
        return Ok(());
    };
    PayloadFormat::from_str(format)
        .map(|_| ())
...
mod tests {
    ...
    fn staged_batch(offset: &str, rows: u64) -> StagedBatch {
        let messages = (0..rows)
            .map(|index| ProducedMessage {
                key: None,
                headers: None,
                timestamp: Some(now_millis()),
                payload: format!("{{\"id\":{index}}}").into_bytes(),
            })
            .collect();
        StagedBatch {
            messages,
            ...
            apply: PendingApply::Tables(Vec::new()),
        }
    }

    #[test]
    fn given_staged_batch_should_attach_post_batch_state_without_committing() {
        let src = PostgresSource::new(1, test_config(), None);

        let runtime = tokio::runtime::Runtime::new().unwrap();
        runtime.block_on(async {
            let produced = src
                .stage_batch(Some(staged_batch("42", 3)))
                .await
                .expect("staging should succeed");

            assert_eq!(produced.messages.len(), 3);
            let attached: State = produced
                .state
                .expect("non-empty batch must carry state")
                .deserialize(CONNECTOR_NAME, 1)
                .expect("attached state should deserialize");
            assert_eq!(
                attached.tracking_offsets.get("users"),
                Some(&"42".to_string())
            );
            assert_eq!(attached.processed_rows, 3);

            let committed = src.state.lock().await;
            assert!(committed.tracking_offsets.is_empty());
            assert_eq!(committed.processed_rows, 0);
            assert!(src.pending.lock().await.is_some());
        });
    }

    #[test]
    fn given_nack_when_batch_is_staged_should_discard_staged_cursor() {
        let src = PostgresSource::new(1, test_config(), None);

        let runtime = tokio::runtime::Runtime::new().unwrap();
        runtime.block_on(async {
            src.stage_batch(Some(staged_batch("42", 3)))
                .await
                .expect("staging should succeed");
            ...
            assert!(committed.tracking_offsets.is_empty());
            assert_eq!(committed.processed_rows, 0);
            assert!(src.pending.lock().await.is_none());
        });
    }

    #[test]
    fn given_ack_when_batch_is_staged_should_apply_staged_cursor() {
        let src = PostgresSource::new(1, test_config(), None);
        ...
        runtime.block_on(async {
            ...
            let committed = src.state.lock().await;
            assert_eq!(
                committed.tracking_offsets.get("users"),
                Some(&"42".to_string())
            );
            assert_eq!(committed.processed_rows, 3);
            assert!(src.pending.lock().await.is_none());
        });
    }
    ...
    fn given_empty_poll_after_nack_should_carry_no_advanced_state() {
        ...
        runtime.block_on(async {
            ...
            assert!(src.pending.lock().await.is_none());
            assert!(src.state.lock().await.tracking_offsets.is_empty());
        });
    }

    #[test]
    fn given_ack_without_staged_batch_should_be_noop() {
        let src = PostgresSource::new(1, test_config(), None);
        ...
        runtime.block_on(async {
            src.on_batch_result(SourceBatchResult::Ack)
                .await
                .expect("ACK without pending batch should succeed");
            assert!(src.state.lock().await.tracking_offsets.is_empty());
        });
    }

    #[test]
    fn given_no_cleanup_config_should_not_stage_table_cleanups() {
        let src = PostgresSource::new(1, test_config(), None);
        assert!(!src.cleans_up_processed_rows());

        let mut config = test_config();
        config.delete_after_read = Some(true);
        assert!(PostgresSource::new(1, config, None).cleans_up_processed_rows());
    ...
    #[test]
    fn given_payload_format_string_should_parse_case_insensitively() {
        assert_eq!(
            PayloadFormat::from_str("BYTEA").unwrap(),
            PayloadFormat::Bytea
        );
        assert_eq!(
            PayloadFormat::from_str("raw").unwrap(),
            PayloadFormat::Bytea
        );
        assert_eq!(
            PayloadFormat::from_str("jsonb").unwrap(),
            PayloadFormat::JsonDirect
        );
        assert_eq!(
            PayloadFormat::from_str("text").unwrap(),
            PayloadFormat::Text
        );
        assert!(matches!(
            PayloadFormat::from_str("xml"),
