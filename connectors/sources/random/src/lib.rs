...
const DEFAULT_USER_POOL: u32 = 10;
...
pub struct RandomSourceConfig {
    ...
    user_pool: Option<u32>,
    key_by_user: Option<bool>,
}
...
impl RandomSource {
    pub fn new(id: u32, config: RandomSourceConfig, state: Option<ConnectorState>) -> Self {
        ...
        RandomSource {
            ...
            user_pool: config.user_pool.unwrap_or(DEFAULT_USER_POOL).max(1),
            key_by_user: config.key_by_user.unwrap_or(false),
    ...
    fn generate_messages(
        ...
        for index in 0..messages_count {
            let sequence = sequence_start + index;
            let user_id = format!("user-{}", sequence % self.user_pool as usize);
            let record = Record {
                id: Uuid::new_v4(),
                sequence,
            ...
            let message = ProducedMessage {
                key: self.key_by_user.then(|| user_id.into_bytes()),
                headers: None,
                timestamp: None,
                payload,
            };
...
impl Source for RandomSource {
    async fn open(&mut self) -> Result<(), Error> {
        info!(
            "Opened random source connector with ID: {}. Interval: {:#?}, max count: {:#?}, messages range: {} - {}, payload size: {}, user pool: {}",
            self.id,
            self.interval,
            self.max_count,
    ...
    async fn poll(&self) -> Result<ProducedMessages, Error> {
        ...
        let messages = self.generate_messages(remaining, messages_produced);
...
mod tests {
    ...
    #[test]
    fn given_user_pool_when_generating_should_cycle_user_ids_and_set_keys() {
        let src = RandomSource::new(1, test_config(), None);
        let messages = src.generate_messages(Some(6), 0);
        assert!((5..=6).contains(&messages.len()));
        for (index, message) in messages.iter().enumerate() {
            let expected = format!("user-{}", index % 3);
            assert_eq!(message.key.as_deref(), Some(expected.as_bytes()));
            let mut payload = message.payload.clone();
            let value = simd_json::to_owned_value(&mut payload).unwrap();
            let record: Record = simd_json::serde::from_owned_value(value).unwrap();
            assert_eq!(record.user_id, expected);
