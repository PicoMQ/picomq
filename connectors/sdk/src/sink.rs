    ...
    pub unsafe fn consume(
        ...
        unsafe {
            ...
            let messages_metadata = match postcard::from_bytes::<MessagesMetadata>(
                ...
                Err(err) => {
                    error!(
                        "Failed to decode messages metadata by sink connector with ID: {} from topic: {}. {err}",
                        self.id, topic_metadata.topic
                    );
                    return -1;
                }
            };
            ...
            let raw_messages = match postcard::from_bytes::<RawMessages>(messages_slice) {
                ...
                Err(err) => {
                    error!(
                        "Failed to decode raw messages by sink connector with ID: {} from topic: {}. {err}",
                        self.id, topic_metadata.topic
                    );
                    return -1;
                }
            ...
            for message in raw_messages.messages {
                let headers = if message.headers.is_empty() {
                    ...
                    match postcard::from_bytes(&message.headers) {
                        ...
                        Err(err) => {
                            error!(
                                "Failed to decode message headers by sink connector with ID: {} from topic: {}. {err}",
                                self.id, topic_metadata.topic
                            );
                            continue;
                        }
                ...
                let payload = match messages_metadata.schema.try_into_payload(message.payload) {
                    ...
                    Err(err) => {
                        error!(
                            "Failed to decode message payload by sink connector with ID: {} from topic: {}. {err}",
                            self.id, topic_metadata.topic
                        );
                        continue;
                    }
            ...
            match result {
                ...
                Err(err) => {
                    error!(
                        "Failed to consume messages by sink connector with ID: {} from topic: {}. {err}",
                        self.id, topic_metadata.topic
                    );
                    1
                }
...
macro_rules! sink_connector {
    ($type:ty) => {
        ...
        #[cfg(not(test))]
        #[unsafe(no_mangle)]
        unsafe extern "C" fn pico_sink_open(
            id: u32,
            config_ptr: *const u8,
            config_len: usize,
            log_callback: LogCallback,
        ...
        #[cfg(not(test))]
        #[unsafe(no_mangle)]
        unsafe extern "C" fn pico_sink_consume(
            id: u32,
            topic_meta_ptr: *const u8,
            topic_meta_len: usize,
            messages_meta_ptr: *const u8,
        ...
        #[cfg(not(test))]
        #[unsafe(no_mangle)]
        unsafe extern "C" fn pico_sink_close(id: u32) -> i32 {
        ...
        #[cfg(not(test))]
        #[unsafe(no_mangle)]
        extern "C" fn pico_sink_version() -> *const std::ffi::c_char {
