...
const MAX_CONSECUTIVE_NACKS: u32 = 30;
pub const POLL_TASK_ENDED_BATCH_ID: u64 = u64::MAX;
...
{
    let ended_by_shutdown = poll_until_stopped(
        plugin_id,
        source,
        &callback,
        &mut shutdown,
    ...
    if !ended_by_shutdown {
        error!(
            "Source connector with ID: {plugin_id} stopped polling; notifying runtime that no more batches will be produced"
        );
        callback(plugin_id, POLL_TASK_ENDED_BATCH_ID, std::ptr::null(), 0);
    }
}

async fn poll_until_stopped<T, F>(
    plugin_id: u32,
    source: Arc<T>,
    callback: &F,
...
macro_rules! source_connector {
    ($type:ty) => {
        ...
        #[cfg(not(test))]
        #[unsafe(no_mangle)]
        unsafe extern "C" fn pico_source_open(
            id: u32,
            config_ptr: *const u8,
            config_len: usize,
            state_ptr: *const u8,
        ...
        #[cfg(not(test))]
        #[unsafe(no_mangle)]
        unsafe extern "C" fn pico_source_handle_v2(id: u32, callback: SendCallback) -> i32 {
        ...
        #[cfg(not(test))]
        #[unsafe(no_mangle)]
        extern "C" fn pico_source_batch_result(id: u32, batch_id: u64, result: u8) -> i32 {
        ...
        #[cfg(not(test))]
        #[unsafe(no_mangle)]
        unsafe extern "C" fn pico_source_close(id: u32) -> i32 {
            let Some(mut instance) = INSTANCES.get_mut(&id) else {
                tracing::error!(
        ...
        #[cfg(not(test))]
        #[unsafe(no_mangle)]
        extern "C" fn pico_source_version() -> *const std::ffi::c_char {
...
mod tests {
    ...
    fn given_batch_result_handler_failure_should_stop_polling() {
        ...
        runtime.block_on(async {
            ...
            let task = tokio::spawn(handle_messages(
                13,
                source_for_task,
                move |_, batch_id, messages_ptr, messages_len| {
                    batch_sender
                        .send((batch_id, messages_ptr.is_null(), messages_len))
                        .expect("batch receiver should remain open");
                    0
                },
                shutdown_receiver,
            ...
            let (batch_id, _, _) =
                tokio::time::timeout(Duration::from_secs(1), batch_receiver.recv())
                    .await
                    .expect("first batch was not sent")
                    .expect("batch channel closed");
            assert_eq!(
            ...
            let sentinel = tokio::time::timeout(Duration::from_secs(1), batch_receiver.recv())
                .await
                .expect("poll task end was not reported")
                .expect("batch channel closed");
            assert_eq!(sentinel, (POLL_TASK_ENDED_BATCH_ID, true, 0));
            assert_eq!(source.polls.load(Ordering::SeqCst), 1);
            assert_eq!(
                *source
                    .results
    ...
    #[test]
    fn given_shutdown_should_not_report_poll_task_end() {
        let runtime = tokio::runtime::Runtime::new().expect("failed to create test runtime");
        runtime.block_on(async {
            let source = Arc::new(TestSource::default());
            let pending_batch = Arc::new(Mutex::new(None));
            ...
            let task = tokio::spawn(handle_messages(
                ...
                Arc::clone(&source),
                move |_, batch_id, _, _| {
                    let _ = batch_sender.send(batch_id);
                    0
                },
                shutdown_receiver,
                pending_batch,
            ...
            tokio::time::timeout(Duration::from_secs(1), batch_receiver.recv())
                ...
                .expect("first batch was not sent")
                .expect("batch channel closed");
            shutdown_sender.send(()).expect("shutdown receiver dropped");
            tokio::time::timeout(Duration::from_secs(1), task)
                .await
                .expect("source task did not stop after shutdown")
                .expect("source task failed");

            while let Ok(batch_id) = batch_receiver.try_recv() {
                assert_ne!(batch_id, POLL_TASK_ENDED_BATCH_ID);
            }
        });
    }
