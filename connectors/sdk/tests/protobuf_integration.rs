use picomq_connector_sdk::decoders::proto::{ProtoConfig, ProtoStreamDecoder};
use picomq_connector_sdk::encoders::proto::{ProtoEncoderConfig, ProtoStreamEncoder};
use picomq_connector_sdk::transforms::{ProtoConvert, ProtoConvertConfig, Transform};
use picomq_connector_sdk::{Payload, Schema, StreamDecoder, StreamEncoder};
use prost::Message;
use prost_types::Any;
...
async fn should_transform_with_real_schema_and_field_mapping() {
    ...
    let metadata = picomq_connector_sdk::TopicMetadata {
        topic: "test_topic".to_string(),
    };

    let input_message = picomq_connector_sdk::DecodedMessage {
        offset: Some(0),
...
async fn should_perform_json_to_proto_to_json_roundtrip() {
    ...
    let original_message = picomq_connector_sdk::DecodedMessage {
        offset: Some(0),
        timestamp: Some(1642771200),
        key: None,
        headers: None,
