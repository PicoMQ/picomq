...
var _ = ginkgo.Describe("Pico groups", func() {
	ginkgo.It("speaks the group wire contract", func() {
		...
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			case 1:
				...
				Expect(readJSON(r)).To(Equal(map[string]any{
					...
					"instanceId":         "i-1",
					"clientId":           "test",
					"sessionTimeoutMs":   float64(6000),
					"rebalanceTimeoutMs": float64(9000),
				}))
				writeJSON(w, http.StatusOK, map[string]any{
					"memberId":   "m/1",
					"generation": 2,
					"assignment": []string{"/a"},
					"members":    []string{"m/1", "m/2"},
				})
			case 2:
				Expect(r.Method).To(Equal(http.MethodPost))
			...
			case 5:
				Expect(readJSON(r)).To(Equal(map[string]any{
					"offsets": map[string]any{"/b": map[string]any{"position": float64(1)}},
				}))
				w.WriteHeader(http.StatusNoContent)
			case 6:
				Expect(r.Method).To(Equal(http.MethodGet))
				...
				writeJSON(w, http.StatusOK, map[string]any{
					"offsets": map[string]any{
						"/a":   map[string]any{"position": 7, "metadata": "ck"},
						"/b c": map[string]any{"position": 1, "metadata": nil},
					},
				})
			case 7:
				Expect(r.Method).To(Equal(http.MethodGet))
			...
			case 8:
				...
				writeJSON(w, http.StatusOK, map[string]any{
					"groups": []map[string]any{{"group": "orders/eu", "state": "Stable"}},
				})
			case 9:
		...
		joined, err := client.JoinGroup(ctx, "orders/eu", []string{"/a", "/b"}, JoinOptions{
			MemberID:         "m/1",
			InstanceID:       "i-1",
			ClientID:         "test",
			SessionTimeout:   6 * time.Second,
			RebalanceTimeout: 9 * time.Second,
		})
		Expect(err).NotTo(HaveOccurred())
		Expect(joined).To(Equal(GroupMembership{
			MemberID:   "m/1",
			Generation: 2,
			Assignment: []string{"/a"},
			Members:    []string{"m/1", "m/2"},
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(described).To(Equal(GroupDescription{
			Group:        "orders/eu",
			State:        "Stable",
			Generation:   2,
			ProtocolType: "consumer",
	...
	ginkgo.It("rejoins on rebalance and publishes the new assignment", func() {
		...
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			case r.Method == http.MethodPost && r.URL.Path == "/_groups/g/members":
				...
				case 1:
					writeJSON(w, http.StatusOK, map[string]any{
						"memberId": "m-1", "generation": 1, "assignment": []string{"/a", "/b"}, "members": []string{"m-1"},
					})
				case 2:
					writeJSON(w, http.StatusOK, map[string]any{
						"memberId": "m-1", "generation": 2, "assignment": []string{"/a"}, "members": []string{"m-1", "m-2"},
					})
				default:
					writeJSON(w, http.StatusOK, map[string]any{
						"memberId": "m-9", "generation": 3, "assignment": []string{"/b"}, "members": []string{"m-9"},
					})
				}
			...
			case r.Method == http.MethodPut && r.URL.Path == "/_groups/g/offsets":
				Expect(readJSON(r)).To(Equal(map[string]any{
					"offsets":    map[string]any{"/b": map[string]any{"position": float64(4)}},
					"memberId":   "m-9",
		...
		config := GroupConfig{
			...
			ClientID:          "go-test",
			Retry:             RetryPolicy{MaxAttempts: 3, Multiplier: 1},
		}
	...
	ginkgo.It("fails the session on a fatal heartbeat error and skips leave", func() {
		...
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			case r.Method == http.MethodPost && r.URL.Path == "/_groups/g/members":
				writeJSON(w, http.StatusOK, map[string]any{
					"memberId": "m-1", "generation": 1, "assignment": []string{"/a"}, "members": []string{"m-1"},
				})
			case r.Method == http.MethodPost && r.URL.Path == "/_groups/g/members/m-1/heartbeat":
	...
	ginkgo.It("fails when retryable heartbeat errors exhaust the policy", func() {
		...
		config := GroupConfig{
			SessionTimeout:    300 * time.Millisecond,
			HeartbeatInterval: 5 * time.Millisecond,
			Retry:             RetryPolicy{MaxAttempts: 2, Multiplier: 1},
		}
