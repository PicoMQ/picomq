...
Stream names are paths, such as `logs/api/prod`. Each name maps to a registry entry stored in the metadata KV, which is part of the replicated state described in [Metadata](/docs/design/metadata). The entry records the internal stream id, the content type, the optional [schema](/docs/schemas) bind, an optional Kafka topic, TTL and expiry, whether the stream is closed, and the state of each idempotent producer.
